...
const MaddenDB: MaddenDB = {
  ...
  getLatestTeams: async function(leagueId: string): Promise<TeamList> {
    const teamDocs = await db.collection("league_data").doc(leagueId).collection(MaddenEvents.MADDEN_TEAM).get()
  ...
  getLatestWeekSchedule: async function(leagueId: string, week: number) {
    const weekDocs = await db.collection("league_data").doc(leagueId).collection(MaddenEvents.MADDEN_SCHEDULE).where("weekIndex", "==", week - 1)
  ...
  getWeekScheduleForSeason: async function(leagueId: string, week: number, season: number) {
    const weekDocs = await db.collection("league_data").doc(leagueId).collection(MaddenEvents.MADDEN_SCHEDULE).where("weekIndex", "==", week - 1).where("seasonIndex", "==", season)
      .where("stageIndex", "==", 1).get()
    const maddenSchedule = weekDocs.docs.filter(d => !d.id.startsWith("schedules")).map(d => d.data() as SnallabotEvent<MaddenGame>)
      .filter(game => game.awayTeamId != 0 && game.homeTeamId != 0)
    if (maddenSchedule.length !== 0) {
    ...
    const allDocs = await db.collection("league_data").doc(leagueId).collection(MaddenEvents.MADDEN_SCHEDULE).get()
    const allGameChanges = await Promise.all(allDocs.docs.map(async (doc) => {
      ...
      const changesSnapshot = await db.collection("league_data").doc(leagueId).collection(MaddenEvents.MADDEN_SCHEDULE).doc(doc.id).collection("history").get()
  ...
  getGameForSchedule: async function(leagueId: string, scheduleId: number, week: number, season: number) {
    const schedule = await db.collection("league_data").doc(leagueId).collection(MaddenEvents.MADDEN_SCHEDULE).doc(`${scheduleId}`).get()
    if (!schedule.exists) {
    ...
    const history = await db.collection("league_data").doc(leagueId).collection(MaddenEvents.MADDEN_SCHEDULE).doc(`${scheduleId}`).collection("history").get()
  ...
  getStandingForTeam: async function(leagueId: string, teamId: number) {
    const standing = await db.collection("league_data").doc(leagueId).collection(MaddenEvents.MADDEN_STANDING).doc(`${teamId}`).get()
    if (!standing.exists) {
  ...
  getLatestStandings: async function(leagueId: string) {
    const standingSnapshot = await db.collection("league_data").doc(leagueId).collection(MaddenEvents.MADDEN_STANDING).get()
  ...
  getLatestPlayers: async function(leagueId: string) {
    const playerSnapshot = await db.collection("league_data").doc(leagueId).collection(MaddenEvents.MADDEN_PLAYER).select("rosterId", "firstName", "lastName", "teamId", "position").get()
  ...
  getPlayer: async function(leagueId: string, rosterId: string) {
    const playerDoc = await db.collection("league_data").doc(leagueId).collection(MaddenEvents.MADDEN_PLAYER).doc(rosterId).get()
    if (playerDoc.exists) {
  ...
  getPlayers: async function(leagueId: string, query: PlayerListQuery, limit, startAfter?: Player, endBefore?: Player) {
    ...
    if (endBefore) {
      playersQuery = db.collection("league_data").doc(leagueId).collection(MaddenEvents.MADDEN_PLAYER).orderBy("playerBestOvr", "asc").orderBy("rosterId", "desc").limit(limit)
    } else {
      playersQuery = db.collection("league_data").doc(leagueId).collection(MaddenEvents.MADDEN_PLAYER).orderBy("playerBestOvr", "desc").orderBy("rosterId").limit(limit)
    }
